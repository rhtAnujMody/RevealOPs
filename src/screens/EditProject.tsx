...
const EditProject: React.FC = () => {
  ...
  const { data: projectData, getProjectDetails, setId } = useProjectDetailsStore();
  ...
  useEffect(() => {
    ...
    fetchCustomers();
    fetchSOWs();
  }, [projectId, setId, getProjectDetails]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiRequest(`${constants.ALL_PROJECTS}${projectId}/`, 'PATCH', formData);
      if (response.ok) {
